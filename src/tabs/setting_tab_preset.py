...
def create_system_preset_management_tab():
    with gr.Tab("시스템 메시지 프리셋 관리"):
        with gr.Row():
            ...
            refresh_preset_button.click(fn=chat_bot.refresh_preset_list, inputs=[app_state.selected_language_state], outputs=[preset_dropdown])
            apply_preset_btn = gr.Button("프리셋 적용")

        with gr.Row():
            preset_name = gr.Textbox(label="새 프리셋 이름", placeholder="예: 친절한 비서", interactive=True)
            preset_content = gr.Textbox(label="프리셋 내용", placeholder="프리셋으로 사용할 시스템 메시지를 입력하세요.", lines=4, interactive=True)

        with gr.Row():
            add_preset_btn = gr.Button("프리셋 추가", variant="primary")
            delete_preset_btn = gr.Button("프리셋 삭제", variant="secondary")

        preset_info = gr.Textbox(label="프리셋 관리 결과", interactive=False)
        ...
        overwrite_message = gr.Textbox(label="덮어쓰기 메시지", value="", interactive=False)

        add_preset_btn.click(fn=on_add_preset_click, inputs=[preset_name, preset_content], outputs=[preset_info, confirm_overwrite_btn, cancel_overwrite_btn, overwrite_message])

        confirm_overwrite_btn.click(fn=confirm_overwrite, inputs=[preset_name, preset_content], outputs=[preset_info, confirm_overwrite_btn, cancel_overwrite_btn, overwrite_message])

        # 덮어쓰기 취소 버튼 클릭 시
        def cancel_overwrite():
            return "❌ 덮어쓰기가 취소되었습니다.", gr.update(visible=False), gr.update(visible=False), ""

        cancel_overwrite_btn.click(fn=cancel_overwrite, inputs=[], outputs=[preset_info, confirm_overwrite_btn, cancel_overwrite_btn, overwrite_message])
        ...
        delete_preset_btn.click(fn=lambda: (gr.update(visible=True), gr.update(visible=True), gr.update(visible=True)), inputs=[preset_dropdown], outputs=[delete_preset_confirm_row, delete_preset_yes_btn, delete_preset_no_btn])
        ...
        # 프리셋 삭제 취소 버튼 클릭 시
        delete_preset_no_btn.click(fn=lambda: ("❌ 삭제가 취소되었습니다.", gr.update(visible=False), preset_dropdown), inputs=[], outputs=[preset_info, delete_preset_confirm_row, preset_dropdown])

        apply_preset_btn.click(fn=apply_preset, inputs=[preset_dropdown, app_state.session_id_state, app_state.history_state, app_state.selected_language_state], outputs=[preset_info, app_state.history_state, ui_component.system_message_box, ui_component.profile_image]).then(
            fn=chat_bot.filter_messages_for_chatbot, inputs=[app_state.history_state], outputs=ui_component.chatbot
        )
