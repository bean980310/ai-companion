...
from huggingface_hub import HfApi, snapshot_download, model_info, login, hf_hub_download
...
def scan_diffusion_models(root="./models/diffusion"):
    ...
    local_models = []
    # 모델 타입 디렉토리 목록
    model_types = ["checkpoints", "transformers", "text_encoders", "clip_vision", "clip", "configs", "controlnet", "diffusers", "embeddings", "loras", "unet", "vae", "vae_approx"]
...
def get_all_local_models():
    ...
    return {"transformers": transformers, "vllm-local": vllm, "gguf": gguf, "mlx": mlx}
...
def get_all_diffusion_models():
    ...
    loras = [m["model_id"] for m in all_models if m["model_type"] == "loras"]
    vae = [m["model_id"] for m in all_models if m["model_type"] == "vae"]
    clip = [m["model_id"] for m in all_models if m["model_type"] == "clip"]
    controlnet = [m["model_id"] for m in all_models if m["model_type"] == "controlnet"]
    text_encoders = [m["model_id"] for m in all_models if m["model_type"] == "text_encoders"]
    unet = [m["model_id"] for m in all_models if m["model_type"] == "unet"]
    transformers = [m["model_id"] for m in all_models if m["model_type"] == "transformers"]
    vae_approx = [m["model_id"] for m in all_models if m["model_type"] == "vae_approx"]
    embeddings = [m["model_id"] for m in all_models if m["model_type"] == "embeddings"]
    clip_vision = [m["model_id"] for m in all_models if m["model_type"] == "clip_vision"]
    configs = [m["model_id"] for m in all_models if m["model_type"] == "configs"]
    return {"diffusers": diffusers, "checkpoints": checkpoints, "loras": loras, "vae": vae, "clip": clip, "controlnet": controlnet, "text_encoders": text_encoders, "unet": unet, "transformers": transformers, "vae_approx": vae_approx, "embeddings": embeddings, "clip_vision": clip_vision, "configs": configs}
...
def get_all_tts_models():
    ...
    omnivoice = [m["model_id"] for m in models if m["model_type"] == "omnivoice"]
    supertonic = [m["model_id"] for m in models if m["model_type"] == "supertonic"]
...
async def download_model_from_hf_async(repo_id: str, target_dir: Optional[str] = None, token: Optional[str] = None, progress_callback: Optional[Callable] = None) -> str:
    """
...
def get_terminators(tokenizer):
    ...
    if "glm" in str(tokenizer.__class__).lower():
        ...
        # Qwen 모델용 특수 처리
        return [tokenizer.eos_token_id, tokenizer.convert_tokens_to_ids("<|im_end|>")]
    else:
        # 기존 다른 모델들을 위한 처리
        return [tokenizer.convert_tokens_to_ids("<|end_of_text|>"), tokenizer.convert_tokens_to_ids("<|eot_id|>"), tokenizer.eos_token_id if hasattr(tokenizer, "eos_token_id") else None]
...
def build_model_cache_key(model_id: str, model_type: str, model_provider: str, lora_model_id: Optional[str] = None, quantization_bit: Optional[str] = None, local_path: Optional[str] = None) -> str:
    """
...
def clear_model_cache(model_id: str, local_path: Optional[str] = None) -> str:
    """
