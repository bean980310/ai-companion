...
try:
    ...
    raise ImportError("Please install the outetts package to use this handler.")
...
class OuteTTSTransformersHandler:
    ...
    def load_model(self):
        self.config = outetts.ModelConfig(model_path=self.local_model_path, tokenizer_path=self.local_model_path, interface_version=outetts.InterfaceVersion.V3, backend=outetts.Backend.HF, device=self.device, dtype=torch.bfloat16)
...
class OuteTTSGGUFHandler:
    def __init__(self, model_id, lora_model_id=None, model_type="gguf", device="cpu", speaker="speaker.json", **kwargs):
        self.model = None
        self.tokenizer = None
        self.config = None
        self.device = device
    ...
    def load_model(self):
        self.config = outetts.ModelConfig(model_path=self.local_model_path, tokenizer_path=self.local_model_path, interface_version=outetts.InterfaceVersion.V3, backend=outetts.Backend.LLAMACPP, device=self.device, dtype=torch.bfloat16)
