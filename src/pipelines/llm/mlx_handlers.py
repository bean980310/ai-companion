...
class MlxVisionModelHandler(BaseVisionModelHandler):
    ...
    def generate_answer(self, history, **kwargs):
        if self.image_input:
            ...
            response = stream_generate(self.model, self.processor, formatted_prompt, self.image_input, verbose=False, temperature=self.temperature, top_p=self.top_p, top_k=self.top_k, repetition_penalty=self.repetition_penalty, max_tokens=self.max_tokens)
