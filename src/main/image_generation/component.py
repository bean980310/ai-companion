    ...
    def create_diffusion_side_refiner_model_container(cls):
        with gr.Row(elem_classes="model-container") as refiner_row:
            with gr.Column():
                ...
                refiner_model_dropdown = gr.Dropdown(label=_("refiner_model_select_label"), choices=app_state.diffusion_refiner_choices, value=app_state.diffusion_refiner_choices[0], elem_classes="model-dropdown")
                refiner_start = gr.Slider(label="Refiner Start Step", minimum=1, maximum=50, step=1, value=20, visible=False)
                with_refiner_image_to_image_start = gr.Slider(label="Image to Image Start Step", minimum=1, maximum=50, step=1, value=20, visible=False)

        ui_component.diffusion_refiner_model_dropdown = refiner_model_dropdown
    ...
    def create_diffusion_side_lora_container(cls):
        with gr.Row(elem_classes="model-container") as lora_row:
            with gr.Accordion("LoRA Settings", open=False, elem_classes="accordion-container"):
                lora_multiselect = gr.Dropdown(label=_("lora_select_label"), choices=app_state.diffusion_lora_choices, value=[], interactive=True, multiselect=True, info="Select LoRA models to apply to the diffusion model.", elem_classes="model-dropdown")
                lora_text_encoder_sliders = []
                lora_unet_sliders = []
                for i in range(app_state.max_diffusion_lora_rows):
                    text_encoder_slider = gr.Slider(label=f"LoRA {i + 1} - Text Encoder Weight", minimum=-2.0, maximum=2.0, step=0.01, value=1.0, visible=False, interactive=True)
                    unet_slider = gr.Slider(label=f"LoRA {i + 1} - U-Net Weight", minimum=-2.0, maximum=2.0, step=0.01, value=1.0, visible=False, interactive=True)
                    lora_text_encoder_sliders.append(text_encoder_slider)
                    lora_unet_sliders.append(unet_slider)
                diffusion_lora_slider_rows = []
                for te, unet in zip(lora_text_encoder_sliders, lora_unet_sliders):
                    diffusion_lora_slider_rows.append(gr.Row([te, unet]))
                for row in diffusion_lora_slider_rows:
                    row

        ui_component.diffusion_lora_multiselect = lora_multiselect
    ...
    def create_diffusion_container_image_to_image_panel(cls):
        with gr.Row(elem_classes="model-container"):
            with gr.Accordion("Image to Image", open=False, elem_classes="accordion-container"):
                image_to_image_mode = gr.Radio(label="Image to Image Mode", choices=["None", "Image to Image", "Inpaint", "Inpaint Upload"], value="None", elem_classes="model-dropdown")
                with gr.Column():
                    with gr.Row():
                        image_to_image_input = gr.Image(label="Image to Image", type="filepath", sources="upload", format="png", visible=False)
                        image_inpaint_input = gr.Image(label="Image Inpaint", type="filepath", sources="upload", format="png", visible=False)
                        image_inpaint_masking = gr.ImageMask(label="Image Inpaint Mask", type="filepath", sources="upload", format="png", visible=False)

                blur_radius_slider = gr.Slider(label="Blur Radius", minimum=0, maximum=10, step=0.5, value=5, visible=False)
                blur_expansion_radius_slider = gr.Slider(label="Blur Expansion Radius", minimum=0, maximum=100, step=1, value=1, visible=False)
                denoise_strength_slider = gr.Slider(label="Denoise Strength", minimum=0.0, maximum=1.0, step=0.01, value=0.5, visible=False)
        ...
        return cls(
            ...
            image_to_image_input=image_to_image_input,
            image_inpaint_input=image_inpaint_input,
            image_inpaint_masking=image_inpaint_masking,
            blur_radius_slider=blur_radius_slider,
            blur_expansion_radius_slider=blur_expansion_radius_slider,
            denoise_strength_slider=denoise_strength_slider,
        )

    @classmethod
    def create_diffusion_container_main_panel(cls):
        with gr.Column(scale=7):
            positive_prompt_input = gr.TextArea(label="Positive Prompt", placeholder="Enter positive prompt...", elem_classes="message-input")
            negative_prompt_input = gr.TextArea(label="Negative Prompt", placeholder="Enter negative prompt...", elem_classes="message-input")

            with gr.Row():
                style_dropdown = gr.Dropdown(label="Style", choices=["Photographic", "Digital Art", "Oil Painting", "Watercolor"], value="Photographic")

            with gr.Row():
                width_slider = gr.Slider(label="Width", minimum=128, maximum=2048, step=64, value=512)
                height_slider = gr.Slider(label="Height", minimum=128, maximum=2048, step=64, value=512)

            with gr.Row():
                generation_step_slider = gr.Slider(label="Generation Steps", minimum=1, maximum=50, step=1, value=20)
            ...
            gallery = gr.Gallery(label="Generated Images", format="png", columns=2, rows=2)
        ...
        return cls(
            positive_prompt_input=positive_prompt_input, negative_prompt_input=negative_prompt_input, style_dropdown=style_dropdown, width_slider=width_slider, height_slider=height_slider, generation_step_slider=generation_step_slider, random_prompt_btn=random_prompt_btn, generate_btn=generate_btn, gallery=gallery
        )
    ...
    def create_diffusion_container_side_panel(cls):
        with gr.Column(scale=3, elem_classes="side-panel"):
            with gr.Accordion(_("advanced_setting"), open=False, elem_classes="accordion-container") as advanced_setting:
                ...
                cfg_scale_slider = gr.Slider(label="CFG Scale", minimum=1, maximum=20, step=0.5, value=7.5)
                with gr.Row():
                    seed_input = gr.Number(label="Seed", value=42, precision=0)
                    random_seed_checkbox = gr.Checkbox(label="Random Seed", value=True)
                with gr.Row() as vae_row:
                    vae_dropdown = gr.Dropdown(label="Select VAE Model", choices=app_state.vae_choices, value="Default", interactive=True, info="Select VAE model to apply to the diffusion model.", elem_classes="model-dropdown")
                with gr.Row():
                    clip_skip_slider = gr.Slider(label="Clip Skip", minimum=1, maximum=10, step=1, value=2)
                    enable_clip_skip_checkbox = gr.Checkbox(label="Enable Custom Clip Skip", value=False)
                    clip_g_checkbox = gr.Checkbox(label="Enable Clip G", value=False)
                with gr.Row():
                    batch_size_input = gr.Number(label="Batch Size", value=1, precision=0)
                    batch_count_input = gr.Number(label="Batch Count", value=1, precision=0)
        ...
        ui_component.diffusion_seed_input = seed_input
        ...
        return cls(
            ...
            vae_dropdown=vae_dropdown,
            clip_skip_slider=clip_skip_slider,
            enable_clip_skip_checkbox=enable_clip_skip_checkbox,
            clip_g_checkbox=clip_g_checkbox,
            batch_size_input=batch_size_input,
            batch_count_input=batch_count_input,
            vae_row=vae_row,
        )

    @classmethod
    def create_diffusion_container_history_panel(cls):
        image_history = gr.Dataframe(headers=["Prompt", "Negative Prompt", "Steps", "Model", "Sampler", "Scheduler", "CFG Scale", "Seed", "Width", "Height"], label="Generation History", col_count=(10, "dynamic"), wrap=True, datatype=["str", "str", "str", "str", "str", "str", "str", "str", "str", "str"])
