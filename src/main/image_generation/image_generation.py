...
def generate_images_wrapper(positive_prompt, negative_prompt, style, generation_step, diffusion_refiner_start, width, height,
...
def generate_images_to_images_wrapper(positive_prompt, negative_prompt, style, generation_step, diffusion_refiner_start,
    diffusion_model, diffusion_refiner_model, diffusion_model_type, lora_multiselect, vae, clip_skip, enable_clip_skip, clip_g, sampler, scheduler,
    batch_size, batch_count, cfg_scale, seed, random_seed, image_input, denoise_strength,
    # 이후 20개의 슬라이더 값 (max_diffusion_lora_rows * 2; 예를 들어 10행이면 20개)
    *lora_slider_values):
    n = len(lora_slider_values) // 2
    text_weights = list(lora_slider_values[:n])
    ...
    if diffusion_refiner_model == "None":
        ...
        clip_g=True
        return generate_images_with_refiner(
            positive_prompt, negative_prompt, style, generation_step, diffusion_refiner_start,
            diffusion_model, diffusion_refiner_model, diffusion_model_type, lora_multiselect, vae, clip_skip, enable_clip_skip, clip_g, sampler, scheduler,
            batch_size, batch_count, cfg_scale, seed, random_seed, image_input, denoise_strength,
            text_weights_json, unet_weights_json
        )