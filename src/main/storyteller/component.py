...
from ..chatbot import ChatbotComponent


@dataclass
class StoryComponent(ChatbotComponent):
    @classmethod
    def create_story_side_model_container(cls):
        with gr.Row(elem_classes="model-container"):
            with gr.Column():
                gr.Markdown("### Model Selection")
                model_provider_dropdown = gr.Dropdown(label=_("model_provider_label"), choices=get_llm_provider_list(), value=get_llm_provider_list()[0], interactive=True, elem_classes="model-dropdown")
        ...
        ui_component.storytelling_model_provider_dropdown = model_provider_dropdown
        ui_component.storytelling_model_type_dropdown = model_type_dropdown
        ui_component.storytelling_model_dropdown = model_dropdown
        ui_component.storytelling_api_key_text = api_key_text
        ui_component.storytelling_lora_dropdown = lora_dropdown
        ui_component.storytelling_refresh_button = refresh_button
        ui_component.story_clear_all_btn = clear_all_btn
...
def create_story_side():
    with gr.Column() as storyteller_side:
        with gr.Row(elem_classes="model-container"):
            with gr.Column():
                gr.Markdown("### Model Selection")
                storytelling_model_type_dropdown = gr.Radio(label=_("model_type_label"), choices=app_state.llm_type_choices, value=app_state.llm_type_choices[0], elem_classes="model-dropdown")
                storytelling_model_dropdown = gr.Dropdown(label=_("model_select_label"), choices=app_state.initial_choices, value=app_state.initial_choices[0] if len(app_state.initial_choices) > 0 else None, elem_classes="model-dropdown")
                storytelling_api_key_text = gr.Textbox(label=_("api_key_label"), placeholder="sk-...", visible=False, elem_classes="api-key-input")
                storytelling_lora_dropdown = gr.Dropdown(label=_("lora_select_label"), choices=get_all_loras(), value="None", interactive=True, visible=False, elem_classes="model-dropdown")
...
def create_story_container_main_panel():
    with gr.Column(scale=7):
        ...
        storytelling_output = gr.Textbox(label="Output", lines=10, elem_classes="message-output")
...
def create_story_container_side_panel():
    with gr.Column(scale=3, elem_classes="side-panel"):
        with gr.Accordion(_("advanced_setting"), open=False, elem_classes="accordion-container") as story_adv_setting:
            storyteller_seed_input = gr.Number(label=_("seed_label"), value=42, precision=0, step=1, interactive=True, info=_("seed_info"), elem_classes="seed-input")
            storyteller_temperature_slider = gr.Slider(label=_("temperature_label"), minimum=0.0, maximum=1.0, value=0.6, step=0.1, interactive=True)
            storyteller_top_k_slider = gr.Slider(label=_("top_k_label"), minimum=0, maximum=100, value=20, step=1, interactive=True)
            storyteller_top_p_slider = gr.Slider(label=_("top_p_label"), minimum=0.0, maximum=1.0, value=0.9, step=0.1, interactive=True)
            storyteller_repetition_penalty_slider = gr.Slider(label=_("repetition_penalty_label"), minimum=0.0, maximum=2.0, value=1.1, step=0.1, interactive=True)
...
def create_story_container():
    ...
    return [story_container, storytelling_input, storytelling_btn, storytelling_output, story_adv_setting, storyteller_seed_input, storyteller_temperature_slider, storyteller_top_k_slider, storyteller_top_p_slider, storyteller_repetition_penalty_slider]
...
def create_story_container_2():
    ...
    return [story_tab, storytelling_input, storytelling_btn, storytelling_output, story_adv_setting, storyteller_seed_input, storyteller_temperature_slider, storyteller_top_k_slider, storyteller_top_p_slider, storyteller_repetition_penalty_slider]
