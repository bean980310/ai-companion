...
ローカル環境向けAIコンパニオンはUNIX/LinuxカーネルのOSに最適化されています。Windowsでも使用は可能ですが、Windowsで直接実行する場合GPUアクセラレーションを利用できません。Windows環境でローカル環境向けAIコンパニオンを適切に使用するには、WSL2環境での利用を推奨します。